...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT_5x7(KC_ESC, KC_1, KC_2, KC_3, KC_4, KC_5, KC_6, KC_TAB, KC_Q, KC_W, KC_E, KC_R, KC_T, KC_LBRC, CW_TOGG, KC_A, KC_S, KC_D, KC_F, KC_G, KC_LPRN, KC_LSFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_LCTL, KC_LGUI, KC_LEFT, KC_RGHT, KC_LSFT, LT(3,KC_DEL), KC_LCTL, KC_LALT, KC_PWR, DM_PLY1, KC_7, KC_8, KC_9, KC_0, KC_MINS, KC_EQL, KC_GRV, KC_RBRC, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_BSLS, KC_RPRN, KC_H, KC_J, KC_K, KC_L, KC_SCLN, KC_QUOT, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT, KC_BTN1, KC_BTN2, KC_PSCR, LGUI(KC_L), LT(2,KC_ENT), KC_SPC, KC_INS, MO(2), DM_PLY2, KC_WREF),
    [1] = LAYOUT_5x7(KC_ESC, UP(ONE, LEK_ATTAK_MUOY), UP(TWO, LEK_ATTAK_PII), UP(THREE, LEK_ATTAK_BEI), UP(FOUR, LEK_ATTAK_BUON), UP(FIVE, LEK_ATTAK_PRAM), XXXXXXX, KC_TAB, UP(KO, KHO), UP(CA, CHA), UP(CO, CHO), UP(NGO, NYO), UP(DA, TTHA), KC_TRNS, CW_TOGG, UP(SA, HA), UP(KA, KHA), UP(NO, NNA), UP(RO, VO), UP(BA, PHA), KC_TRNS, KC_LSFT, UP(DO, TTHO), UP(TA, THA), UP(TO, THO), UP(PO, PHO), UP(YO, QA), KC_LCTL, KC_LGUI, KC_LEFT, KC_RIGHT, KC_LSFT, LT(3,KC_DEL), KC_LCTL, KC_LALT, KC_ENTER, DM_PLY1, XXXXXXX, UP(SIX, LEK_ATTAK_PRAM_MUOY), UP(SEVEN, LEK_ATTAK_PRAM_PII), UP(EIGHT, LEK_ATTAK_PRAM_BEI), UP(NINE, LEK_ATTAK_PRAM_BUON), UP(ZERO, LEK_ATTAK_SON), UP(KOOMUUT, PHNAEK_MUAN), KC_TRNS, UP(OE, YA), UP(I, IE), UM(II), UP(Y, LY), UP(YY, LYY), UM(NIKAHIT), KC_TRNS, UP(E, OO), UP(AA, AU), UM(COENG), UM(MO), UP(LO, LA), UP(BANTOC, MUUSIKATOAN), UP(AE, AI), UM(U), UM(UU), UM(UA), UP(REAHMUK, YUUKALEAPINTU), UP(KHAN, QMARK), MS_BTN1, MS_BTN2, KC_PSCR, LGUI(KC_L), LT(2,KC_ENT), UP(SPACE, ZWS), MO(5), MO(2), DM_PLY2, KC_WREF),
    ...
    [5] = LAYOUT_5x7(KC_TRNS, UP(MUOY_ROC, DAP_MUOY_ROC), UP(PII_ROC, DAP_PII_ROC), UP(BEI_ROC, DAP_BEI_ROC), UP(BUON_ROC, DAP_BUON_ROC), UP(PRAM_ROC, DAP_PRAM_ROC), KC_TRNS, KC_TRNS, UM(TRIISAP), UM(BANTOC), UM(ROBAT), UM(TOANDAKHIAT), KC_TRNS, KC_TRNS, KC_TRNS, UM(KAKABAT), UM(AHSDA), UM(SAMYOK_SANNYA), UM(VIRIAM), UM(ATTHACAN), KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, UM(PRAM_MUOY_ROC), UM(PRAM_PII_ROC), UM(PRAM_BEI_ROC), UM(PRAM_BUON_ROC), UM(DAP_ROC), UM(LEK_TOO), KC_TRNS, KC_TRNS, UM(QI), UM(QII), UM(RY), UM(RYY), UM(BARIYOOSAN), KC_TRNS, UM(QOO_TYPE_ONE), UM(QAU), UM(QE), KC_TRNS, KC_TRNS, UM(RIEL), UM(QAI), UM(QU), UM(QUU), UM(QUUV), UM(CAMNUC_PII_KUUH), KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS)
};
